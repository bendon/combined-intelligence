...
$url = "https://tpu.googleapis.com/v2/$parent/acceleratorTypes?pageSize=500"
$headers = @{ "Authorization" = "Bearer $AccessToken" }

Write-Host "Accelerator types in location: $Zone (project: $Project)" -ForegroundColor Cyan
$r = Invoke-TpuRestWithErrorBody -Method Get -Uri $url -Headers $headers

if ($r.acceleratorTypes -and $r.acceleratorTypes.Count -gt 0) {
  foreach ($t in $r.acceleratorTypes) {
    $name = $t.name
    if ($name -match '/acceleratorTypes/([^/]+)$') { $name = $Matches[1] }
    Write-Host ("  " + $name)
  }
}
